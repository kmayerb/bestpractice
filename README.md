## bestpractice
[![Build Status](https://travis-ci.org/uwescience/bestpractice.svg?branch=master)](https://travis-ci.org/uwescience/bestpractice)
...
instructions at the [bottom of this page](#using-bestpractice-as-a-template).
...
`bestpractice/bestpractice.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `bestpractice.py` in directory called
`bestpractice`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import bestpractice as sb` in an
interactive Python session, the classes and functions defined inside of the
`bestpractice.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the bestpractice module, the main classes defined at the bottom of
...
Either way, you can create a `bestpractice/data` folder in which you can
...
(typically, something like our `bestpractice/tests/test_bestpractice.py`). Within each
...
have one such test in `bestpractice/tests/test_bestpractice.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/bestpractice`),
and type:

    py.test bestpractice

This will exercise all of the tests in your code directory. If a test fails, you
will see a message such as:


    bestpractice/tests/test_bestpractice.py .F...
    ...
    bestpractice/tests/test_bestpractice.py:49: AssertionError
...
To document `bestpractice` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/bestpractice/blob/master/doc/_static/RTD-advanced-conf.png)

 http://bestpractice.readthedocs.org/en/latest/
...
A `bestpractice/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/bestpractice) for the software. This
...
`bestpractice` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `bestpractice`, we use the
...
[here](https://github.com/uwescience/bestpractice/blob/master/scripts/Figure1.ipynb).
...
### Using `bestpractice` as a template
...
Under "old repository clone URL" enter: https://github.com/uwescience/bestpractice
...
`bestpractice/bestpractice.py` to be called `smallish/smallish.py`

	git mv bestpractice smallish
	git mv smallish/bestpractice.py smallish/smallish.py
	git mv smallish/tests/test_bestpractice.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `bestpractice` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `bestpractice` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `bestpractice` in them. Type
the following to see where all these files are:

	git grep bestpractice

You can replace `bestpractice` for `smallish` quickly with:

	git grep -l 'bestpractice' | xargs sed -i 's/bestpractice/smallish/g'
...
Edit all the mentions of `bestpractice` in the `bestpractice/__init__.py` file, and in
the `bestpractice/version.py` file as well.
